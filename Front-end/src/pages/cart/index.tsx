						...
						<div className='cart-container'>
									...
									return (
										<ul className='item-info' key={item._id}>
											<li className='title-item'>
												<i className='fa-solid fa-trash icon-remove' onClick={() => handleRemoveItem(item)}></i>
											...
											<li className='item-quantity'>
												<Button onClick={() => handleUpdateCart(item, item.quantity + 1)} disabled={item.quantity === item.countInStock}>
											...
											<li className='item-price-total'>${item.quantity * item.price}</li>
										</ul>
									)
								})}
							</div>
							...
							<div className='total-box'>
								...
								<p className='total-price'>
									Total
									<span>${cartItems.reduce((acc, value) => acc + value.price * value.quantity, 0)}</span>
								</p>
